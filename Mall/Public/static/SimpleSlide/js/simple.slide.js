...
(function ($) {
    $.fn.simpleSlide = function (options) {
        ...
        var fooo=_ATTR.fooo;
        if(_ATTR.fooo!=0){
            fooo=fooo.split(',');//字符串转数组
        }
        //获取当前ID
        var _i = _LOOK_INDEX;
        // var _i;_ATTR.lookIndex== undefined ? _i = _LOOK_INDEX : _i=_ATTR.lookIndex;
        // console.log(_i);
        // console.log(_ATTR.lookIndex);
        ...
        if(_ATTR.fooo==0){
            ...
            for (var j = 0; j < fooo.length; j++) {
                // arrObj[j]=$('#aid'+fooo[j]).attr("i");
                $('#aid'+fooo[j]).attr("i") == undefined ? _ALL_IMG_URL[j] = undefined : _ALL_IMG_URL[j] = $('#aid'+fooo[j]).attr("i");
                if ($('#aid'+fooo[j]).attr("i") == undefined) {
                    _I++;
                }
            }
            console.log(_ALL_IMG_URL);

            var arrObj=[];
            _THIS.each(function (i) {
                _OBJ = $(this);
                arrObj[i]=_OBJ.attr("id").substr(3);
            });
            console.log(arrObj);  //原始节点数组
            console.log(fooo);    //更换顺序数组
        ...
        _THIS.on("dblclick",function (event) {
            ...
            //获取当前的点击元素的索引
            _IMG_INDEX = _THIS.index($(this));
            console.log(_IMG_INDEX);

            for (var i = 0; i <= arr2.length; i++) {
                if (_IMG_INDEX==i) {
                    _IMG_INDEX=arr2[_IMG_INDEX];
                    break;
                }
            }
            console.log(arr2);
            console.log(_IMG_INDEX);
            //如果当前元素不存在 i 属性 则返回一个正确的URL
        ...
        function _SET_WINDOWS() {
            ...
            //循环添加图片
            for (var i = 0; i < _ALL_IMG_URL.length; i++) {
                $("#LookPicture_" + _i).append('<img alt="No (i) attribute or URL error" class="Look_img pannable-image animated ' + _ATTR.imageAction + '" src="' + _ATTR.loadingImage + '" iSrc="' + _ALL_IMG_URL[i] + '" />');
                // _ATTR.loadingImage
            }
            //添加上一页按钮
